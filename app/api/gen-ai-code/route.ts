...
export async function POST(request: NextRequest) {
  ...
  // Limit based on the user's unique Clerk ID so they can't spam generations
  ...
  const cached = await getCached(lastUserMessage.content, fileData);
    ...
    async start(controller) {
      ...
      try {
        ...
        if (cached) {
          ...
          enqueue(sseEvent("status", { message: "Loading cached result…" }));
          parsed = cached;
        } else {
          // ── Cache miss — call Gemini as before ────────────────────────────
          ...
          await setCached(lastUserMessage.content, fileData, parsed);
        }
